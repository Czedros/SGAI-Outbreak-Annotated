...
class Board:
    ...
    # Initialization method for the board of the game itself. ignore self for all cases. dimension is a 2 value array that dictates the size of the board.
    ...
    #checks for the number of zombies on the board
        ...
        for state in self.States:
            if state.person != None:
                if state.person.isZombie:
                    r += 1
        return r
    #Performs an action depending on the state of the board.
    def act(self, oldstate, givenAction):
        ...
        if f[0] == False:
            reward = reward * 0
        return [reward, f[1]]
    #Finds the first zombie
    def containsZombie(self):
        for state in self.States:
            if state.person is not None and state.person.isZombie == True:
                return True
        return False
    #Finds whether a position contains a person
    ...
    def get_possible_moves(self, action, role):
        """
        Get the coordinates of people (or zombies) that are able to make the specified move.
    ...
    def isAdjacentTo(self, coord, is_zombie: bool) -> bool:
        ...
        return ret
    #The Movement action for both Humans and Zombies
    ...
    # The Move Up, Down, Left, and Right actions respectively
    ...
    #Completes the Zombie Side Bite action
    def bite(self, coords):
        i = self.toIndex(coords)
        if self.States[i] is None:
            return False
        p = self.States[i].person
        chance = p.calcInfection()
        r = rd.randint(0, 100)
        if r < chance:
            newP = p.clone()
            newP.isZombie = True
    ...
    #Completes the Government Side Heal/Vaccinate action depending on who you use the heal on.
    ...
    #Populates the board with Zombies and Humans
    def populate(self):
